...
class DenonAVRVolume(DenonAVRFoundation):
    ...
    def _max_volume_setup_callback(self, zone: str, event: str, parameter: str) -> None:
        ...
        This is the only trustworthy telnet source for the limit. MVMAX looks
        like a second one but reports the hardware ceiling in some states and a
        stale limit in others, indistinguishable from a real one.
    ...
    async def async_volume_up(self) -> None:
        ...
        # nothing at all, so repeating it is pure noise. Compared against None
        # rather than truthiness: 0.0 is a legal limit and a legal volume.
