...
# Every entry needs a reason. A family is delegated on the branch that lands its
# read, not before -- delegating a setter whose getter stays None is how
# async_speaker_preset_toggle came to send the same value every time.
...
# Volume limits in the AppCommand samples, as <limit> reports them. OFF means
# no limit is configured and must read back as None, not as a float.
MAX_VOLUMES = {
    "NR1609": {"Main": None, "Zone2": -20.0},
    "AVC-8500H": {"Main": None, "Zone2": -10.0, "Zone3": -10.0},
    "AVC-A10H": {"Main": -20.0, "Zone2": -10.0, "Zone3": -10.0},
    "AVR-X4300H": {"Main": None, "Zone2": -10.0, "Zone3": -10.0},
    # SR6012 reports the main zone limit as "  0.0", padded with whitespace
    "SR6012": {"Main": 0.0, "Zone2": -10.0},
    "AVC-X3700H": {"Main": None, "Zone2": -10.0},
...
# still carries values. Models declaring more than one subwoofer report one
# value per subwoofer.
SUBWOOFER_LEVELS = {
